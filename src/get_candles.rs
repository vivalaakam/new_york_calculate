...
pub fn perform_candles(
    ...
    for row in &in_day {
        ...
        let next_time = [3600, 3600 * 3, 3600 * 6, 3600 * 12, 3600 * 24];
        let mut max_value = vec![0f64; 5];
        let mut prev = row.start_time;
        let mut max = 0f64;

        for i in 0..next_time.len() {
            let next_stamp = row.start_time + next_time[i];

            let next = candles_cache
                .iter()
                .filter(|k| k.start_time >= prev && k.start_time < next_stamp)
                .collect::<Vec<_>>();

            for candle in next {
                max = max.max(candle.high / row.open)
            }
        ...
        candle.max_profit = max_value.into_iter().map(|v| (v - 1f64) * 100f64).collect();
        candle.max_profit_1 = candle.max_profit[0];
        candle.max_profit_3 = candle.max_profit[1];
        candle.max_profit_6 = candle.max_profit[2];
        candle.max_profit_12 = candle.max_profit[3];
        candle.max_profit_24 = candle.max_profit[4];
...
mod tests {
    ...
    async fn test_perform_candles_without_indicators() {
        ...
        assert_eq!(
            format!("{:?}", candles[0]),
            r#"Candle { start_time: 1655769600, end_time: 1655769899, open: 0.3226, high: 0.3232, low: 0.3217, close: 0.3223, volume: 1047813.0, quote: 337795.6648, trades: 491.0, buy_base: 502532.0, buy_quote: 161987.0611, history: [], shape: [12, 0], max_profit: [0.7439553626782391, 1.332920024798523, 1.7668939863608069, 2.9758214507129566, 4.587724736515808], max_profit_1: 0.7439553626782391, max_profit_3: 1.332920024798523, max_profit_6: 1.7668939863608069, max_profit_12: 2.9758214507129566, max_profit_24: 4.587724736515808, score: 0.0, interval: 5 }"#
        );
